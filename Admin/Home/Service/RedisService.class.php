...
class RedisService extends  Service{
    ...
    protected   $configname = 'REDIS_NEW_CONFIG';
    /**
     * 构造函数
     */
    ...
    {
    	$config = C($this->configname);
    	if(!$config){
    		$this->configname = 'REDIS_NEW_CONFIG';
    		$config = C($this->configname);
    	}
    	...
    	$ret = $this->_linkHandle[$des]->pconnect($config[$des]['host'],$config[$des]['port']);
    	if($config[$des]['pwd']){
    ...
    public function t_set($key, $value, $expire=0){
    	$this->getRedis($key)->set('a',1);
    	//var_dump($this->getRedis($key));exit;
        // 永不超时
        if($expire == 0){
	 ...
	 */
	public function t_keys($keyword)
	{
		if (strstr($keyword, '*'))
		{
			$rs = array();
	...
	public function __call($method,$args){
		$slave = array('get'=>1,'exists'=>1,'keys'=>1,'lsize'=>1,'lget'=>1,'lrange'=>1,'lset'=>1,'ltrim'=>1,'lrem'=>1,'hget'=>1,'hgetall'=>1,'hexists'=>1,'hlen'=>1,'hvals'=>1,'ttl'=>1);
	...
	public function t_lGet($key,$index=0){
		return $this->getRedis($key)->lGet($key, $index);
	}
