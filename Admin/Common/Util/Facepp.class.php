...
class Facepp
{
    ...
    public function execute($method, array $params)
    {
        ...
        return $this->request("{$this->server}{$method}",$method, $params);
    }

    private function request($request_url,$method, $request_body)
    {
    	$log = service('Log');
    	//$redis->incr('facepp_'.$method.'_'.date('Ymd').'_all',1);//每次访问将次数加1
    	$log->setFaceCount('facepp_'.$method,'all',date('Ymd'));
        ...
        if($response_header['http_code'] == 200){
        	$log->setFaceCount('facepp_'.$method,'success',date('Ymd'));
        }else{
        	$log->setFaceCount('facepp_'.$method,'error',date('Ymd'));
        }
