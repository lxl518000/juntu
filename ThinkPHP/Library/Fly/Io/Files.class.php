 ...
 * 文件及文件夹处理类
 * +----------------------------------------------------------------------
 * @version $Id: Files.class.php 6 2016-01-12 01:35:23Z wangjin $
 */
namespace Org\Io;
...
class Files {
	 ...
	 */
	public static function checkDirs($dirs, $create = true)
	{
		if(file_exists($dirs)) {
			return true;
		} else {
	 ...
	 * @desc 删除文件夹
	 */
	public function deleteDir($dir, $level = 0)
	{
		$dh = @opendir($dir);
		while ($file = @readdir($dh)) {
			if ($file != '.' && $file != '..') {
				...
				if (!is_dir($fullpath)) {
					...
					self :: deleteDir($fullpath, $level + 1);
				}
			}
		}
		@closedir($dh);
