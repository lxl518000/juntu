...
var FormValid = function(frm,show) {
    this.frm = frm;
	this.show = show;
    ...
    this.passed = function() {
        if (this.errMsg.length > 0) {
            FormValid.showError(this.errMsg,this.show,this.errName);
...
FormValid.showError = function(errMsg,show,errName) {
	var msg = "";
	...
	error_show(errMsg,errName);
	}
	//return ;
}
function CheckForm(frm,show) {
	var formElements = frm.elements;
	var fv = new FormValid(frm,show);
	for (var i=0; i<formElements.length;i++) {
		...
		for (var j=0; j<vts.length; j++) {
			...
			if(curErrorMsg == undefined)continue;
